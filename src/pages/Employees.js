...
import { Plus, Edit, Trash2, Search, UserCheck, UserX, ChevronLeft, ChevronRight, ArrowUpDown } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSend = {
        ...
        nightshift_rate: formData.nightshift_rate ? parseFloat(formData.nightshift_rate) : null,
      };
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-amber-500 to-orange-500 text-white">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-semibold cursor-pointer hover:bg-amber-600" onClick={() => handleSort('name')}>
                  <div className="flex items-center space-x-1">
                    <span>Name</span>
                    <ArrowUpDown className="w-4 h-4" />
                  </div>
                </th>
                <th className="px-6 py-4 text-left text-sm font-semibold cursor-pointer hover:bg-amber-600" onClick={() => handleSort('role')}>
                  <div className="flex items-center space-x-1">
                    <span>Role</span>
                    <ArrowUpDown className="w-4 h-4" />
                  </div>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold cursor-pointer hover:bg-amber-600" onClick={() => handleSort('salary_rate')}>
                  <div className="flex items-center space-x-1">
                    <span>Rate</span>
                    <ArrowUpDown className="w-4 h-4" />
                  </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {employees.length === 0 ? (
                ...
                employees?.map((employee) => (
                  <tr key={employee.id} className="hover:bg-amber-50 dark:hover:bg-slate-700/50 transition-colors" data-testid={`employee-row-${employee.id}`}>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Night Shift Rate</label>
                  <input
                    type="number"
                    step="0.01"
